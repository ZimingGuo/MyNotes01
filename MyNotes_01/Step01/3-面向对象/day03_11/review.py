    ...
    类和对象
        ...
        类成员：
            ...
            静态方法:
                实例方法操作实例变量，表示"个体"行为.
                类方法操作类变量，表示"大家"行为.
                静态方法不能操作数据，表示为函数都可以.（可以理解为一个放在了类里的函数）
"""
...
# 通过对象访问
m01 = MyClass(100)
m01.b = 1 #不建议 而且只有 m01 这个对象可以有b 因为实在自己的对象的那块内存里加进去的
m01.print_self() # 调方法
...
# 通过类名访问
print(MyClass02.a)
MyClass02.print_self() # 这两种访问方式都可以


# ------------不常用的访问方式---------------
# 访问实例方法，还可以通过类。
MyClass.print_self(m01)  # 也必须传递对象
# 正常应该是这样的： m01 = MyClass(100)
#                 m01.print_self
