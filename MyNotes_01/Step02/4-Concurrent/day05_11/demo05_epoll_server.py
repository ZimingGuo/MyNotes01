...
# 关注 s（关注IO）
ep.register(s, EPOLLIN | EPOLLERR)  # 关注 s 的两个事件

# 循环监控 IO 发生
while True:
    events = ep.poll()  # 这是一个阻塞函数，是用来监控 IO 事件发生的，监控的就是 register 的 IO 事件
